...
void menu::draw() {
	...
	render::text(5, y, render::fonts::tahoma, "ZadeCheats", false, color(255, 150, 150));

	option_bool(x, &y, "ESP:", &variables::esp, VK_NUMPAD7);
	option_bool(x, &y, "Bunnyhop:", &variables::bunnyhop, VK_NUMPAD8);
	option_int(x, &y, "Chams:", &variables::chams, std::vector<std::string>{ "OFF", "Visible", "Wallhacks" }, VK_NUMPAD9);
	option_bool(x, &y, "RankESP:", &variables::reveal_competitive_ranks, VK_NUMPAD4);
	option_bool(x, &y, "Backtrack:", &variables::backtrack, VK_NUMPAD5);
	option_bool(x, &y, "BTChams:", &variables::backtrack_chams, VK_NUMPAD6);
